...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const fetchAllApi = async (movieId) =>{
    movieId = "1290466";
    console.log("inside fetch all api");
      await celebDetailsApi(movieId);
  ...
  const celebDetailsApi = async(movieId) =>{
    console.log("inside celebDetailsApi");
    const apiUrl = `https://api.themoviedb.org/3/person/${movieId}?language=en-US`;
    ...
    try{
     const result = await apiFetch(apiUrl);
      ...
      console.log("celeb",result);
      
      const movieTitle = result.name;
      const movieOverview = result.biography;
      const releaseYear = result.known_for_department;
    ...
    //   document.getElementById("release-date").innerText = formatReleaseDate(movieReleaseDate);
  ...
  const movieCastApi = async (movieId) =>{
    ...
    try{
      ...
      let directingNames = crewList.filter(person => person.known_for_department=="Directing");
      document.getElementById("director-name").innerText = directingNames[0].name;
      document.getElementById("director-name-bts").innerText = directingNames[0].name;

      let writersNames = crewList.filter(person => person.known_for_department=="Writing");
      // console.log(writersNames);
      document.getElementById("writers-name").innerText = writersNames[0].name;
      document.getElementById("writers-name-bts").innerText = writersNames[0].name;
